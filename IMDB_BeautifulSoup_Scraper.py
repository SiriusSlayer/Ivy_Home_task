...
def get_movies(genre_link):
    ...
    _movie_links = [
        (imdb + movie_links[i]["href"]).split("/") for i in range(0, 2 * n, 2)
    ]
...
def get_movie(movie_link):
    ...
    movie_name = soup.find("h1", attrs={"data-testid": "hero__pageTitle"})
    movie_year_ = movie_name.find_next_sibling("ul")
    
    # Try to find the release year of the movie, If the year is not found, set it as -1
    try:
        movie_year=movie_year_.find('a')
        year = int(movie_year.text)
...
def get_reviews(movie_link):
    ...
    for i in range(1, int(2 * n), 2):
        ...
        review_title_elements = reviews_elements[i].find("a", attrs={"class": "title"})
